...
parser.add_argument('reflist', type=str,
                    help='list of the reference sequences to compare')
...
parser.add_argument('-tk', '--taxKingdom', type=str,
                    default='-',
                    help='taxon at kingdom level')
parser.add_argument('-tp', '--taxPhylum', type=str,
                    default='-',
                    help='taxon at phylum level')
parser.add_argument('-tc', '--taxClass', type=str,
                    default='-',
                    help='taxon at class level')
parser.add_argument('-to', '--taxOrder', type=str,
                    default='-',
                    help='taxon at order level')
parser.add_argument('-tf', '--taxFamily', type=str,
                    default='-',
                    help='taxon at family level')
parser.add_argument('-tg', '--taxGenus', type=str,
                    default='-',
                    help='taxon at genus level')
parser.add_argument('-ts', '--taxSpecies', type=str,
                    default='-',
...
def insert_ani_edges(graph, fname):
    ...
    with open(fname, 'rt') as fh:
        for line in fh:
	    # ANI output format:
            # $ bin/get_ani_identity.sh queries/NC_028464.1.fna CoplaDB.lst
            # queries/NC_028464.1.fna	databases/Copla_RS84/NC_028464.1.fna	100.000	0.000	165	165
            # queries/NC_028464.1.fna	databases/Copla_RS84/NC_010643.1.fna	97.546	3.385	122	165
            # queries/NC_028464.1.fna	databases/Copla_RS84/NC_009982.1.fna	99.773	0.331	158	165
            # queries/NC_028464.1.fna	databases/Copla_RS84/NC_010716.1.fna	97.120	2.937	133	165
            items = line.split("\t")
...
def insert_fastani_edges(graph, fname):
    ...
    with open(fname, 'rt') as fh:
	    ...
	    # FastANI output format:
            # $ bin/get_fastani_identity.sh queries/NC_028464.1.fna CoplaDB.lst
...
# If not provided use prodigal to get the ORFeome. As per Prokka example we use 100Kb to use Prodigal autolearning mode
if args.aminoacid:
...
with open(fname_fna+'.qry_info.tsv', 'w') as fh:
    ...
    fh.write("{}\t{}\t{}\t{}\n".format(g.vp.Size[v_qry], g.vp.MOB[v_qry], g.vp.MPF[v_qry], g.vp.PFinder[v_qry]))
...
#    overlap_strict = float('nan')
#    mutual_strict = float('nan')
#    reduced_strict = float('nan')
#else:
#    overlap_strict = gt.partition_overlap(ref_strict, new_strict, norm=True)
#    mutual_strict = gt.mutual_information(ref_strict, new_strict, norm=True)
#    reduced_strict = gt.reduced_mutual_information(ref_strict, new_strict, norm=True)
