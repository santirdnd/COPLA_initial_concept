...
my $num_args = $#ARGV + 1;
if ($num_args != 2) {
    print "\nUsage: get_ani_identity.pl qry_path ref_lst\n";
    exit;
}
...
#system("cat $ref_list | parallel -j $threads_conf bin/get_ani_identity_job.pl $fname_tmp $len_qry {}");
system("cat $ref_list | parallel bin/get_ani_identity_job.pl $fname_tmp $len_qry {}");
...
for my $fname (@files) {
    ...
    $line = $fname_qry . "\tdatabases/Copla_RS84/" . substr($fname, 0, length($fname) - 4) . "\t" . $data;
    print $OUT $line;
}
close $OUT;
